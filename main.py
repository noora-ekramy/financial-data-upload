...
for i in range(1, num_years + 1):
    ...
    for j in range(1, num_extra + 1):
        ...
        if extra_file:
            if extra_file.name.endswith(".pdf"):
                extra_text = extract_pdf_text(extra_file)
    ...
    if missing:
        ...
        if income_file.name.endswith(".pdf"):
            income_text = extract_pdf_text(income_file)
            income_df = pd.DataFrame({"Extracted_Text": income_text.split("\n")})
        else:
            income_df = pd.read_csv(income_file)

        if balance_file.name.endswith(".pdf"):
            balance_text = extract_pdf_text(balance_file)
            balance_df = pd.DataFrame({"Extracted_Text": balance_text.split("\n")})
        else:
            balance_df = pd.read_csv(balance_file)

        if cash_file.name.endswith(".pdf"):
            cash_text = extract_pdf_text(cash_file)
            cash_df = pd.DataFrame({"Extracted_Text": cash_text.split("\n")})
        else:
